"""Rauchtests der Streamlit-Oberfläche per AppTest: Standard, jedes Preset, Aufgabenwechsel, Abspielen, Permalink, beide Experimente auf Abruf, Schlüssel."""
...
from lgbm_presets import PRESET_KEYS
...
def test_default_renders_without_exception_and_gives_one_verdict():
    at = _run()
    assert any("LightGBM in Aktion" in m.value for m in at.markdown) and not at.error
...
def test_extreme_settings_render():
    def small(at):
        ...
        at.session_state["num_leaves_slider"] = C.NUM_LEAVES_MIN
        at.session_state["n_rounds_slider"] = C.N_ROUNDS_MIN
        at.session_state["n_noise_slider"] = 0
        at.session_state["max_bin_slider"] = C.MAX_BIN_MIN

    def big(at):
        at.session_state["task_select"] = "reg"
        at.session_state["n_slider"] = C.N_MAX
        at.session_state["num_leaves_slider"] = C.NUM_LEAVES_MAX
        at.session_state["n_rounds_slider"] = 80
        at.session_state["n_noise_slider"] = C.NOISE_MAX
        at.session_state["max_bin_slider"] = C.MAX_BIN_MAX
...
def test_step_slider_returns_to_the_last_round_when_settings_change():
    at = _run()
    at.slider(key="lgbm_step").set_value(5)
    at.run()
    assert at.slider(key="lgbm_step").value == 5
    at.session_state["n_rounds_slider"] = 50
    at.run()
    assert not at.exception and at.slider(key="lgbm_step").value == 50


def test_every_round_of_a_small_ensemble_renders():
    at = _run(lambda a: a.session_state.__setitem__("n_rounds_slider", 6))
    for k in range(1, int(at.slider(key="lgbm_step").max) + 1):
        at.slider(key="lgbm_step").set_value(k)
        at.run()
...
def test_permalink_parameters_are_clamped():
    ...
    at.query_params["nl"] = "-4"
    at.query_params["fx"] = "99"
    at.run()
    assert not at.exception
    assert at.slider(key="n_rounds_slider").value == C.N_ROUNDS_MAX and at.slider(key="num_leaves_slider").value == C.NUM_LEAVES_MIN
...
def test_policy_experiment_runs_on_demand():
    at = _run()
    assert not any("blattweise jagt hier eher dem Rauschen" in c.value for c in at.caption)
    at.button(key="policy_start").click()
    at.run()
    assert not at.exception, [e.value for e in at.exception]
    text = " ".join(c.value for c in at.caption)
    assert "blattweise jagt hier eher dem Rauschen" in text


def test_counter_experiment_runs_on_demand():
    at = _run()
    at.button(key="counter_start").click()
    at.run()
    assert not at.exception, [e.value for e in at.exception]
    text = " ".join(c.value for c in at.caption)
    assert "sogar MEHR Kandidaten" in text
...
def test_every_plotly_chart_has_an_explicit_key_and_axes_are_locked():
    ...
    assert sorted(set(keys)) == sorted(["tree_chart", "map_chart", "round_chart", "importance_chart", "policy_chart", "counter_chart"]), keys
    looped = [c for c in calls if 'key=f"' in c]
    assert len(looped) == 2 and all('_{current}"' in c for c in looped)
    viz = (ROOT / "lgbm_visualization.py").read_text(encoding="utf-8")
